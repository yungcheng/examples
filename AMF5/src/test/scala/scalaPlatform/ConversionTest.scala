...
class ConversionTest extends AsyncFlatSpec with should.Matchers {
  ...
  "AMF" should "convert a RAML 1.0 API to OAS 2.0" in {
    raml10Client.parse("file://src/test/resources/examples/banking-api.raml") map { parseResult =>
      val transformResult = oas20Client.transformCompatibility(parseResult.baseUnit, ProvidedMediaType.Oas20)
      val renderResult = oas20Client.render(transformResult.baseUnit, "application/oas20+json")
  ...
  "AMF" should "convert an OAS 2.0 API to RAML 1.0" in {
    oas20Client.parse("file://src/test/resources/examples/banking-api.json") map { parseResult =>
      val transformResult = raml10Client.transformCompatibility(parseResult.baseUnit, ProvidedMediaType.Raml10)
      val renderResult = raml10Client.render(transformResult.baseUnit, "application/raml10+yaml")
